...
class Scraper {
	...
	async retrieveAllItemLinks() {
		...
		const categoryLinks = await Link.find({ origin: this.origin, type: 'category', processed: false });
	...
	async retrieveAllItems() {
		...
		console.log('%s item are in database.', await this.Item.find({origin: this.origin}).countDocuments());
	}
